...
export function DownloaderForm({ onMetadataReceived }: DownloaderFormProps) {
  ...
  const getVideoInfo = async (url: string) => {
    try {
      const response = await axios.post(`${API_URL}/api/download/youtube/info`, { url })
  ...
  const downloadVideo = async (url: string) => {
    try {
      const response = await axios.post(`${API_URL}/api/download/youtube`, { url }, {
        responseType: 'json'
      })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full bg-gradient-to-r from-red-500 to-red-600 text-white py-2 px-4 rounded-lg font-medium hover:from-red-600 hover:to-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isLoading ? (
